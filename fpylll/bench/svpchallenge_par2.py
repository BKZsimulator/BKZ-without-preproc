...
# time python -u svpchallenge_par2.py 90 30 1
...
def svp_improve_trial(filename, bs):
    ...
    print "#  BKZ-%d, file %s, time %.2f" %(bs, filename, BKZ_TIME)  
    #print filename, 
    #print_basis_stats(bkz.M, n)
    ENUM_START = time()
    success = enum_trial(bkz, BKZ_TIME, r[0]*.99)
    ENUM_TIME = time() - ENUM_START
    print "#  SVP-%d, file %s, time %.2f, succ. %s" %(bs, filename, ENUM_TIME, success) 
...
START = time()
for dim in range(start_dim, start_dim+1, 2):
    A_pre = IntegerMatrix.from_file("/home/sbai/suite/sb_fpylll/bench/svpchallenge/svpchallengedim%dseed0.txt"%dim)
    #print "---------------------", A_pre.nrows
    ASVP_START = time()
    LLL.reduction(A_pre)
