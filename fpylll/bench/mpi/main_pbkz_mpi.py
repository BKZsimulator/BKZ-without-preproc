...
def main_pbkz_mpi_master (filename, bs, cores):
    ...
    pbkz_mpi = PBKZ_MPI(A, cores)
    pbkz_mpi.lll_obj()
    print pbkz_mpi.lll_obj.delta
    pbkz_mpi.M.update_gso()
    r = [log(pbkz_mpi.M.get_r(i, i)) for i in range(0, pbkz_mpi.A.nrows)]
    print "# starting r "
    print r
    
    params = BKZ.Param (block_size=bs, max_loops=20000,
                            min_success_probability=.01,
                            flags=BKZ.BOUNDED_LLL|BKZ.DUMP_GSO,
                            dump_gso_filename="gso_output.file")
    #and BKZ.DUMP_GSO))
    #and BKZ.GH_BND)
    pbkz_mpi(params=params, min_row=0)
    #print "# done. found sv", pbkz_mpi.M.B[0]
    ...
    pbkz_mpi.M.update_gso()
    r2 = [log(pbkz_mpi.M.get_r(i, i)) for i in range(0, pbkz_mpi.A.nrows)]
    print pbkz_mpi.A[0]
    print "# ending r"
    print r2
